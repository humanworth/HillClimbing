...
class board:
  def __init__(self, list=None,numberOfQueens=8):
    ...
    if list == None:
      ...
      #Placing queens in random cells in every row
      for i in range(0,self.n_queens):
        #while 1:
          rand_col = random.randint(0,self.n_queens-1)
          if self.board[i][rand_col] == 0:
            self.board[i][rand_col] = 1
            #break
  """
...
class queens:
 ...
 #/2
  ...
  def getBestBoard(self):
    ...
    for queenRow in range(0,self.n_queens): #Move one queen in its row stochastically and find the corresponding heuristic cost
      for queenColumn in range(0,self.n_queens):
        if self.chessBoard.board[queenRow][queenColumn] == 1: #get the lowest cost by moving this queen
          for m_row in range(0,self.n_queens):
              randomCol=random.randint(0, self.n_queens-1)      # randomly choose the position of queen to implement stochastic Hill Climbing
              if self.chessBoard.board[m_row][randomCol] != 1:
                  tempBoard = copy.deepcopy(self.chessBoard)
                  tempBoard.board[queenRow][queenColumn] = 0
                  tempBoard.board[m_row][randomCol] = 1
